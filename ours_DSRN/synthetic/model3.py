...
class SpatialRegressor3(nn.Module):
    ...
    def forward(self, u, mask):
        ...
        x_main = self.bn1(input)
        x_main = self.linear1_before_aggr(x_main)
        self.x_main = self.activation1_before_aggr(x_main)

        x_main_1 = self.bn12(self.x_main)
        x_main_1 = self.dropout1_aggr(x_main_1)
        ...
        x_main_2 = self.bn2(x_main)
        x_main_2 = self.dropout12_aggr(x_main_2)
        ...
        # (batch * neighbors, features)
        x_attn = self.bn1_attn(input)
        x_attn = self.linear1_attn(x_attn)
        self.x_attn = self.activation1_attn(x_attn) #nn.ReLU()

        x_attn_1 = self.bn12_attn(self.x_attn)
        x_attn_1 = self.dropout1_attn(x_attn_1)
